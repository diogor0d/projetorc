...
void* power_udp_receiver_thread_func(void* arg) {
    ...
    while (keep_receiver_thread_running) {
        ...
        if (!keep_receiver_thread_running) break;
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3 || argc > 4) { // Progname + ServerIP + ServerPort (+ PSK opcional)
        print_usage(argv[0]);
        return 1;
    }
    ...
    // Chamada a init_protocol MODIFICADA
    ...
    char command[64];
    // char arg1_str[256], arg2_str[256], arg3_str[256], arg4_str[256], arg5_str[256]; // Menos args agora
    ...
    while (fgets(input_line, sizeof(input_line), stdin) != NULL) {
        ...
        if (strcmp(command, "quit") == 0) { /* ... como antes ... */ break; }
        else if (strcmp(command, "help") == 0) { /* ... como antes ... */ print_commands(); }
        else if (strcmp(command, "send") == 0) {
            // Formato MODIFICADO: send <IP_Destino> <mensagem>
            // A porta de destino é a POWER_UDP_PORT_CLIENT padrão
            ...
            if (dest_ip_str && msg_start && strlen(msg_start) > 0) {
                if (strlen(msg_start) > MAX_MESSAGE_USER -1) {
                    ...
                    if (bytes_sent > 0) { /* ... como antes ... */ }
                    else if (bytes_sent == -1) { /* ... como antes ... */ }
                    else if (bytes_sent == -2) { /* ... como antes ... */ }
                    else { /* ... como antes ... */ }
                }
            } else {
                printf("Uso: send <IP_Destino> <mensagem>\n");
            }
        } else if (strcmp(command, "config") == 0) { /* ... como antes, já correspondia à API ... */
            ...
            // Chamada a get_last_message_stats MODIFICADA
            ...
            if (get_last_message_stats(&retransmissions_val, &delivery_time_val) == 0) {
                ...
                if (delivery_time_val >= 0) {
                    ...
                    // A API do PDF não especifica como get_last_message_stats indica falha.
                    // Em power_udp.c, last_msg_delivery_time_ms é -1 em caso de falha.
                    // E last_msg_attempt_successful (interno) também guarda isso.
                    // Aqui podemos assumir que tempo < 0 é falha, mas precisaria ser documentado.
        ...
        } else if (strcmp(command, "loss") == 0) { /* ... como antes, já correspondia à API ... */
            int probability;
    ...
    // ... (limpeza de threads e close_protocol como antes) ...
