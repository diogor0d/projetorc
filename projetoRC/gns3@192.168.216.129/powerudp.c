...
int init_protocol(const char *server_ip, int server_tcp_port_param, const char *psk) {
    ...
    if (setsockopt(multicast_socket, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) < 0) {
        ...
        // Erro comum: "No such device" se o routing multicast não estiver configurado ou interface errada
        // Ou "Invalid argument" se MULTICAST_ADDRESS não for um endereço multicast válido
        close(server_tcp_socket); server_tcp_socket = -1;
        close(udp_socket_internal); udp_socket_internal = -1;
        close(multicast_socket); multicast_socket = -1;
        return -1;
    ...
    if (pthread_create(&multicast_thread_id, NULL, multicast_listener_thread_func, NULL) != 0) {
        ...
        setsockopt(multicast_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq, sizeof(mreq)); // Usar mreq aqui
        close(server_tcp_socket); server_tcp_socket = -1;
        close(udp_socket_internal); udp_socket_internal = -1;
        close(multicast_socket); multicast_socket = -1;
        return -1;
    ...
    if (multicast_socket != -1) {
        ...
        if (inet_pton(AF_INET, MULTICAST_ADDRESS, &mreq_drop.imr_multiaddr.s_addr) > 0) {
            ...
            if (setsockopt(multicast_socket, IPPROTO_IP, IP_DROP_MEMBERSHIP, &mreq_drop, sizeof(mreq_drop)) < 0) {
                // Não fatal, mas registar. Pode acontecer se não se juntou corretamente.
...
int send_message(const char *destination_ip, const char *message, int len) {
    ...
    while (attempts <= internal_power_udp_state.max_retries) {
        ...
        if (!internal_power_udp_state.retransmission_enabled) {
            ...
            last_msg_delivery_time_ms = (time_end_send.tv_sec - time_start_send.tv_sec) * 1000 +
                                            (time_end_send.tv_usec - time_start_send.tv_usec) / 1000;
            return len; // Retornar número de bytes de payload enviados
        }
        ...
        if (ret_select < 0) { // Erro no select()
            if (errno == EINTR) { // Interrompido por um sinal
                ...
                // Para simplificar, continuar para a próxima tentativa (ou tentar a atual novamente).
                // Isto pode precisar de tratamento mais fino se sinais forem esperados.
                continue; // Isto irá efetivamente tentar o select novamente com o mesmo timeout
            }
...
int receive_message(char *buffer, int bufsize) {
    if (!protocol_initialized || udp_socket_internal < 0) { // Verificar inicialização
        ...
        return -2; // Código de erro especial
    }
    ...
    while(protocol_initialized) { // Verificar protocol_initialized na condição do loop
        ...
        if (!protocol_initialized) { // Verificar se o protocolo foi fechado enquanto bloqueado
             ...
             return 0; // Ou um código de erro específico indicando encerramento
        }

        if (bytes_received < 0) { // Erro no recvfrom
            if (errno == EINTR) continue; // Interrompido por sinal, tentar recvfrom novamente
            if (errno == EAGAIN || errno == EWOULDBLOCK) { // Não deveria acontecer com socket bloqueante, a menos que timeout esteja definido
                 // Se o socket tiver timeout, isto é normal. Caso contrário, é um problema.
                 // Por agora, assumir bloqueante, então este é um erro inesperado.
                 perror("[PowerUDP Error] recvfrom data packet (EAGAIN/EWOULDBLOCK on blocking socket?)");
                 return -1;
            }
            ...
            return -1; // Erro genuíno
        }
        if (bytes_received == 0) { // Não deveria acontecer com datagramas UDP
            printf("[PowerUDP Warning] recvfrom retornou 0 bytes.\n");
            continue;
        }
        ...
        // Estamos apenas interessados em pacotes DATA aqui. ACKs/NAKs são para send_message.
        if (received_packet.header.type == PACKET_TYPE_ACK || received_packet.header.type == PACKET_TYPE_NAK) {
            // Estes são processados pelo loop select de send_message. Se chegarem aqui,
            // significa que send_message não estava à espera ou perdeu-os. Apenas ignorar.
            // printf("[PowerUDP Info] ACK/NAK (type %d, seq %u) ignorado por receive_message.\n",
            //       received_packet.header.type, ntohl(received_packet.header.sequence_number));
            continue;
        }
        ...
        if (received_packet.header.type == PACKET_TYPE_DATA) {
            ...
            if (data_len == 0) { // Pacote de dados vazio?
                ...
                // Decidir se isto é aceitável ou deve ser um erro/ignorado.
                // Por agora, processar (e enviar ACK).
            }
             if (data_len > (unsigned int)bufsize) { // Payload excede buffer da aplicação
                ...
                // Enviar NAK se retransmissões estiverem ativadas, conforme PDF (rejeitar mensagem)
            ...
            if (internal_power_udp_state.sequence_enabled) {
                if (recv_seq_num_host == internal_power_udp_state.expected_recv_sequence_number) {
                    ...
                    // PDF: "quando o recetor receber uma mensagem com o número de sequência ... fora de ordem,
                    // deverá rejeitar a mesma, transmitindo uma mensagem NACK ao emissor."
                    if (internal_power_udp_state.retransmission_enabled) { // NAK só faz sentido se o emissor retransmitir
                        power_udp_packet_t nak_packet;
                        nak_packet.header.type = PACKET_TYPE_NAK;
                        // NAK para o número de sequência *recebido* fora de ordem, conforme PDF "rejeitar a mesma"
                ...
                send_ack_flag = 1;      // Ainda enviar ACK se retransmissões estiverem ativadas, mesmo se sequência estiver desligada
                                        // (embora ACK sem sequência possa ser menos útil para o emissor)
            }
            ...
            if (accept_packet_flag) {
                ...
                // buffer[data_len] = '\0'; // Opcional: terminar com nulo se for uma string, boa prática.
                                         // O chamador deve estar ciente do tamanho e tratar como dados binários se necessário.
                return data_len; // Retornar para a aplicação
            }
            // Se não for aceite (ex: fora de ordem), voltar ao loop para esperar por outro pacote
            continue;
    ...
    // Este ponto é alcançado se protocol_initialized se tornar falso durante o loop
...
int get_last_message_stats(int *retransmissions_out, int *delivery_time_ms_out) {
    ...
    // delivery_time_ms_out deve refletir sucesso/falha.
    // A API do PDF é `int *delivery_time`. Se for negativo, pode implicar falha.
    // O código client.c já verifica `delivery_time_val >= 0`.
    if (delivery_time_ms_out) {
        if (last_msg_attempt_successful) { // Se a última tentativa foi bem-sucedida
            *delivery_time_ms_out = last_msg_delivery_time_ms;
            ...
            // O PDF não especifica o que delivery_time deve ser em caso de falha.
            // Definir como -1 para falha é uma convenção comum.
            *delivery_time_ms_out = -1;
        }
    }
    // Retornar 0 para sucesso ao obter estatísticas, -1 se não houver estatísticas válidas.
    return 0;
}
